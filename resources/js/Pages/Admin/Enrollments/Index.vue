<template>
    <div class="container mt-4">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
            <h2 class="fw-bold text-uppercase text-primary mb-3 mb-md-0">Enrollments</h2>
        </div>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle text-center">
                        ...
                        <tbody>
                            <tr v-for="(enrollment, index) in enrollments.data" :key="index">
                                <td class="fw-semibold text-wrap">{{ enrollment.user.name }}</td>
                                ...
                                <td>
                                    <div class="d-flex flex-wrap justify-content-center gap-2">
                                        <Link :href="route('admin.enrollments.show', enrollment.id)" class="btn btn-info btn-sm shadow">
                                            View
                                        </Link>
                                        <Link :href="route('admin.enrollments.edit', enrollment.id)" class="btn btn-warning btn-sm shadow">
                                            Edit
                                        </Link>
                                        <button @click="destroy(enrollment.id)" class="btn btn-danger btn-sm shadow">
                                            Delete
                                        </button>
                                    </div>
                                </td>
        ...
        <!-- Pagination Section -->
        <div v-if="enrollments.links && enrollments.links.length > 1" class="d-flex justify-content-center mt-4">
...
<script setup>
defineProps({ enrollments: Object });
